...
public class SoundStressTests
{
    ...
    [UnityTest]
    public IEnumerator Test24_MemoryUnloadAudioAssets()
    {
        // Load multiple assets
        var clips = new List<AudioClip>();
        for (int i = 0; i < 3; i++) // Reduced from 5
        {
            var clip = AudioClip.Create($"TempClip_{i}", 44100, 1, 44100, false);
            clips.Add(clip);
        }
        
        // Store reference to one clip
        var keptClip = clips[0];
        
        // Clear references to others but keep them from being garbage collected during test
        var tempClips = new List<AudioClip> { clips[1], clips[2] };
        ...
        Assert.IsTrue(keptClip != null, "Referenced clip should not be unloaded");
    ...
    [UnityTest]
    public IEnumerator Test25_CPU_100Plus3DSounds()
    {
        int movingSoundCount = 50; // Reduced from 100
        var movingSources = new List<AudioSource>();
        
        // Create many 3D sounds
        for (int i = 0; i < movingSoundCount; i++)
        {
            var source = CreateAudioSource($"MovingSound_{i}");
            source.spatialBlend = 1f;
            // Position sounds in a circle
            float angle = i * (360f / movingSoundCount);
        ...
        // Move listener rapidly for 1.5 seconds while monitoring performance
        float startTime = Time.time;
        int frames = 0;
        
        while (Time.time - startTime < 1.5f) // Reduced from 2f
        {
            // Move listener in circle
            float angle = Time.time * 180f; // Slower rotation
            audioManager.transform.position = new Vector3(
        ...
        float fps = frames / 1.5f;
        Assert.IsTrue(fps > 20f, $"Should maintain reasonable FPS (was: {fps})");
        Debug.Log($"3D Sound stress test: {fps} FPS average with {movingSoundCount} sounds");
    }
